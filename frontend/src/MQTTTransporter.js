 ...
 * @extends {Transporter}
 */
class MqttTransporter extends BaseTransporter {
  /**
  ...
  connect() {
    return new this.broker.Promise((resolve, reject) => {
      ...
      client.on("message", (rawTopic, buf) => {
        const topic = rawTopic.substring(
          this.prefix.length + this.topicSeparator.length
        );
  ...
  getTopicName(cmd, nodeID) {
    return (
      ...
      this.topicSeparator +
      cmd +
      (nodeID ? this.topicSeparator + nodeID : "")
    );
  }
